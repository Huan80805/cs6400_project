...
class VectorStore:
    ...
    def get_vectors_and_product_ids_from_prefiltered_ids(
        self, filtered_ids_set: Set[int]
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        For prefiltering, we will first locate the vectors matching the filtered product_ids,
        then construct a temporary faiss index on the fly.
        This helper method retrieves the matching vectors for later index construction.
        """
        ...
        for pid in filtered_ids_set:
            idx = self.product_id_to_idx.get(pid)
            if idx is not None:
                found_vectors.append(self.vectors[idx])
                found_pids.append(pid)

        if not found_vectors:
            return np.array([]), np.array([])

        sub_vectors = np.vstack(found_vectors).astype("float32")
        sub_ids = np.array(found_pids, dtype=np.int64)

        return sub_vectors, sub_ids
